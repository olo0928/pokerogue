...
export const modifierTypes = {
  POKEBALL: () => new AddPokeballModifierType("pb", PokeballType.POKEBALL, 25),
  GREAT_BALL: () => new AddPokeballModifierType("gb", PokeballType.GREAT_BALL, 20),
  ULTRA_BALL: () => new AddPokeballModifierType("ub", PokeballType.ULTRA_BALL, 15),
  ...
  NUGGET: () => new MoneyRewardModifierType("modifierType:ModifierType.NUGGET", "nugget", 10, "modifierType:ModifierType.MoneyRewardModifierType.extra.small"),
  BIG_NUGGET: () => new MoneyRewardModifierType("modifierType:ModifierType.BIG_NUGGET", "big_nugget", 25, "modifierType:ModifierType.MoneyRewardModifierType.extra.moderate"),
  RELIC_GOLD: () => new MoneyRewardModifierType("modifierType:ModifierType.RELIC_GOLD", "relic_gold", 100, "modifierType:ModifierType.MoneyRewardModifierType.extra.large"),
  ...
  VOUCHER: () => new AddVoucherModifierType(VoucherType.REGULAR, 20),
  VOUCHER_PLUS: () => new AddVoucherModifierType(VoucherType.PLUS, 10),
  VOUCHER_PREMIUM: () => new AddVoucherModifierType(VoucherType.PREMIUM, 5),
...
const modifierPool: ModifierPool = {
  ...
  [ModifierTier.GREAT]: [
    ...
    new WeightedModifierType(modifierTypes.MAP, (party: Pokemon[]) => party[0].scene.gameMode.isClassic && party[0].scene.currentBattle.waveIndex < 180 ? 50 : 0, 50),
    ...
    new WeightedModifierType(modifierTypes.VOUCHER, (party: Pokemon[], rerollCount: integer) => !party[0].scene.gameMode.isDaily ? Math.max(10 - rerollCount, 8) : 0, 10),
  ].map(m => {
  ...
  [ModifierTier.ROGUE]: [
    ...
    new WeightedModifierType(modifierTypes.LOCK_CAPSULE, skipInLastClassicWaveOrDefault(60)),
    new WeightedModifierType(modifierTypes.SUPER_EXP_CHARM, skipInLastClassicWaveOrDefault(8)),
    new WeightedModifierType(modifierTypes.RARE_FORM_CHANGE_ITEM, (party: Pokemon[]) => Math.min(Math.ceil(party[0].scene.currentBattle.waveIndex / 5), 3) * 32, 96),
    new WeightedModifierType(modifierTypes.MEGA_BRACELET, (party: Pokemon[]) => Math.min(Math.ceil(party[0].scene.currentBattle.waveIndex / 5), 3) * 32, 96),
    ...
    new WeightedModifierType(modifierTypes.VOUCHER_PLUS, (party: Pokemon[], rerollCount: integer) => !party[0].scene.gameMode.isDaily ? Math.max(10 - rerollCount * 1, 8) : 0, 10),
  ].map(m => {
  ...
  [ModifierTier.MASTER]: [
    ...
    new WeightedModifierType(modifierTypes.SHINY_CHARM, 56),
    new WeightedModifierType(modifierTypes.HEALING_CHARM, 18),
    new WeightedModifierType(modifierTypes.MULTI_LENS, 9),
    new WeightedModifierType(modifierTypes.VOUCHER_PREMIUM, (party: Pokemon[], rerollCount: integer) => !party[0].scene.gameMode.isDaily && !party[0].scene.gameMode.isEndless && !party[0].scene.gameMode.isSplicedOnly ? Math.max(10 - rerollCount * 1, 8) : 0, 10),
    new WeightedModifierType(modifierTypes.DNA_SPLICERS, (party: Pokemon[]) => !party[0].scene.gameMode.isSplicedOnly && party.filter(p => !p.fusionSpecies).length > 1 ? 24 : 0, 12),
...
const enemyBuffModifierPool: ModifierPool = {
  [ModifierTier.COMMON]: [
    ...
    new WeightedModifierType(modifierTypes.ENEMY_DAMAGE_REDUCTION, 1),
    ...
    new WeightedModifierType(modifierTypes.ENEMY_ENDURE_CHANCE, 1),
  ...
  [ModifierTier.ULTRA]: [
    ...
    new WeightedModifierType(modifierTypes.ENEMY_DAMAGE_REDUCTION, 1),
    new WeightedModifierType(modifierTypes.ENEMY_HEAL, 1),
    new WeightedModifierType(modifierTypes.ENEMY_STATUS_EFFECT_HEAL_CHANCE, 1),
    new WeightedModifierType(modifierTypes.ENEMY_ENDURE_CHANCE, 1),
...
function getNewModifierTypeOption(party: Pokemon[], poolType: ModifierPoolType, tier?: ModifierTier, upgradeCount?: integer, retryCount: integer = 0): ModifierTypeOption | null {
  ...
  if (tier === undefined) {
    ...
    if (player && tierValue) {
      ...
      const upgradeOdds = Math.floor(64 / ((partyLuckValue + 8) / 4));
      let upgraded = false;
      do {
    ...
    if (tier < ModifierTier.MASTER) {
      ...
      const upgradeOdds = Math.floor(32 / ((partyShinyCount + 4) / 2));
